    ...
    for (let hour = start; hour < end; hour++) {
      for (let minute = 0; minute < 60; minute += 30) {
        ...
        if (today) {
          ...
          if (slotTime < threeHoursFromNow) continue
        }
        times.push(timeStr)
      }
    }
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${step >= 1 ? "bg-rose-600 text-white" : "bg-gray-200 text-gray-600"}`}>1</div>
              <span className="text-sm font-medium">Выбор услуги</span>
            </div>
            <div className="flex-1 h-px bg-gray-300 mx-4" />
            <div className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${step >= 2 ? "bg-rose-600 text-white" : "bg-gray-200 text-gray-600"}`}>2</div>
              <span className="text-sm font-medium">Дата и время</span>
            </div>
            <div className="flex-1 h-px bg-gray-300 mx-4" />
            <div className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${step >= 3 ? "bg-rose-600 text-white" : "bg-gray-200 text-gray-600"}`}>3</div>
          ...
          <form onSubmit={handleSubmit}>
            {step === 1 && (
              <Card>
                <CardHeader><CardTitle>Выберите мастера и услугу</CardTitle></CardHeader>
                <CardContent className="space-y-6">
                  <div>
                    <Label htmlFor="master">Мастер</Label>
                    <Select value={selectedMaster?.id} onValueChange={handleMasterSelect}>
                      <SelectTrigger id="master"><SelectValue placeholder="Выберите мастера" /></SelectTrigger>
                      <SelectContent>
                        {masters.map(m => <SelectItem key={m.id} value={m.id}>{m.name} - {m.specialty}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </div>
                  <div>
                    ...
                    <Select value={selectedService?.id} onValueChange={handleServiceSelect}>
                      ...
                      <SelectContent>
                        {services.map(s => <SelectItem key={s.id} value={s.id}>{s.name} - {s.price} ₽ ({s.duration_minutes} мин)</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </div>
                  {selectedService && (
                  ...
                  <Button type="button" onClick={handleNextStep} disabled={!selectedMaster || !selectedService} className="w-full bg-rose-600 hover:bg-rose-700">Далее</Button>
                </CardContent>
              </Card>
            )}
            {step === 2 && (
              <Card>
                <CardHeader><CardTitle>Выберите дату и время</CardTitle></CardHeader>
                <CardContent className="space-y-6">
                  <div>
                    <Label>Дата</Label>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-4 gap-2 mt-2">
                        {availableTimes.map(time => (
                          <Button key={time} type="button" variant={selectedTime === time ? "default" : "outline"} onClick={() => setSelectedTime(time)} className={selectedTime === time ? "bg-rose-600 hover:bg-rose-700" : ""}>{time}</Button>
                        ))}
                      </div>
                    </div>
                  )}
                  {selectedDate && availableTimes.length === 0 && <p className="text-center text-gray-600 py-4">На выбранную дату нет свободного времени. Выберите другую дату.</p>}
                  <div className="flex gap-4">
                    <Button type="button" variant="outline" onClick={() => setStep(1)} className="flex-1">Назад</Button>
                    <Button type="button" onClick={() => setStep(3)} disabled={!selectedDate || !selectedTime} className="flex-1 bg-rose-600 hover:bg-rose-700">Далее</Button>
                  </div>
                </CardContent>
              </Card>
            )}
            {step === 3 && (
              <Card>
                <CardHeader><CardTitle>Ваши контактные данные</CardTitle></CardHeader>
                <CardContent className="space-y-6">
                  <div><Label htmlFor="name">Имя *</Label><Input id="name" value={clientName} onChange={(e) => setClientName(e.target.value)} required placeholder="Введите ваше имя" /></div>
                  <div><Label htmlFor="email">Email *</Label><Input id="email" type="email" value={clientEmail} onChange={(e) => setClientEmail(e.target.value)} required placeholder="example@mail.com" /></div>
                  <div><Label htmlFor="phone">Телефон *</Label><Input id="phone" type="tel" value={clientPhone} onChange={(e) => setClientPhone(e.target.value)} required placeholder="+7 (999) 123-45-67" /></div>
                  <div><Label htmlFor="notes">Комментарий (необязательно)</Label><Textarea id="notes" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Дополнительные пожелания или комментарии" rows={3} /></div>
                  <div className="flex items-start gap-2">
                    <input type="checkbox" id="consent" checked={consent} onChange={(e) => setConsent(e.target.checked)} required />
                    <Label htmlFor="consent" className="text-sm">
                      Я даю согласие на обработку моих персональных данных в соответствии с <Link href="/privacy" className="text-rose-600 hover:underline">Политикой</Link>
                    </Label>
                  </div>
                  <div className="bg-rose-50 p-4 rounded-lg">
                  ...
                  <div className="flex gap-4">
                    <Button type="button" variant="outline" onClick={() => setStep(2)} className="flex-1">Назад</Button>
                    <Button type="submit" disabled={isLoading} className="flex-1 bg-rose-600 hover:bg-rose-700">{isLoading ? "Создание записи..." : "Подтвердить запись"}</Button>
                  </div>
                </CardContent>
              </Card>
            )}
